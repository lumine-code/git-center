...
// into view, and resets the selection to the first row: every `update({items})`
// re-filters, and re-filtering selects `initialSelectionIndex`. Snapshot updates
// are ambient rather than user navigation, so a status refresh arriving while
// the list is open must not move what the user has highlighted, nor the
// viewport. Rows are rebuilt each pass, so the selection is restored by id
// rather than by identity — and restored before the scroll, since selecting
...
async function updateListPreservingScroll(selectListView, props) {
  ...
  const selectedId = selectListView.getSelectedItem()?.id;
  await selectListView.update(props);
  if (selectedId != null) {
    const restored = selectListView.items?.find((item) => item?.id === selectedId);
    if (restored) selectListView.selectItem(restored);
  }
