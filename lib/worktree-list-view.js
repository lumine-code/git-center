...
module.exports = class WorktreeListView {
  constructor() {
    ...
    this.selectListView = lumine.workspace.buildSelectList({
      ...
      confirmAction: (item) => this.confirmActionForItem(item),
  ...
  // The item-actions list is derived from the commands registered on the
  ...
  confirmActionForItem(item) {
    if (!item) return null;
    if (item.action === "create") return "git-center:create-worktree";
    if (item.action === "prune") return "git-center:prune-worktrees";
    return "git-center:open-worktree-in-this-window";
  }

  actionAppliesToSelection(command) {
    const item = this.selectListView.getSelectedItem();
    if (!item) return false;
    if (item.action) return command === this.confirmActionForItem(item);
    if (command === "git-center:create-worktree" || command === "git-center:prune-worktrees") {
      return false;
    }
    if (command === "git-center:lock-worktree") return !item.locked;
    if (command === "git-center:unlock-worktree") return Boolean(item.locked);
    return true;
  }

  performSelectedAction(action) {
    if (this.selectListView.getSelectedItem()?.action !== action) return;
