...
module.exports = class BranchListView {
  constructor() {
    ...
    this.referenceListHost = lumine.workspace.addSelectList(
      {
        items: [],
    ...
    this.referenceList = this.referenceListHost.getModel();

    this.subscriptions.add(
      this.selectListHost.onDidOpen(() => this.observeActiveRepository()),
      this.selectListHost.onDidHide(() => this.stopObservingActiveRepository()),
      this.referenceListHost.onDidOpen(() => this.observeReferenceRepository()),
      this.referenceListHost.onDidHide(() => this.stopObservingReferenceRepository()),
    );
  }
  ...
  async requestReferenceRefresh() {
    if (!this.referenceListHost.isVisible()) return Promise.resolve();
    const scrollTop = this.referenceList.getScrollTop();
    await this.referenceList.reload();
    await this.referenceList.update({});
    this.referenceList.setScrollTop(scrollTop);
  }
  ...
  async showReferenceList(action, repository) {
    ...
    await this.referenceListHost.show({
      crumb: ACTIONS.find((entry) => entry.action === action).crumb,
    });
    await this.referenceList.update({});
  }
  ...
  destroy() {
    ...
    return Promise.all([
      ...
      this.referenceListHost.destroy(),
